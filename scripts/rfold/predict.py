    ...
    def predict(self, a3m_fn, out_prefix, hhr_fn=None, atab_fn=None, window=150, shift=75):
        ...
        with torch.no_grad():
            ...
            if L > window*2:
                ...
                pair_s = np.zeros((1, L, L, MODEL_PARAM["d_pair"]), dtype=np.float32)
                pair_g = np.zeros((1, L, L, MODEL_PARAM["d_pair"]), dtype=np.float32)
                ...
                for i in range(ngrids):
                    for j in range(i, ngrids):
                        ...
                        with torch.cuda.amp.autocast():
                            logit_s, node, init_crds, pred_lddt, pair = self.model(input_msa, input_seq, input_idx, t1d=input_t1d, t2d=input_t2d, return_raw=True)
                        #
                        ...
                        del logit_s, node
                        pair_s[:, sub_idx_2d[0], sub_idx_2d[1]] += pair.cpu().numpy()
                        pair_g[:, sub_idx_2d[0], sub_idx_2d[1]] += 1.0
                #
                # combine all crops
                for i in range(4):
                ...
                if np.any(pair_g == 0):
                    print("FIXING zero pair_g")
                    pair_g[pair_g == 0] = 1.0
                pair_s = pair_s / pair_g
                #
...
#         print(prob_s[0].dtype)
        ...
        np.savez_compressed("%s.feature.npz"%(out_prefix), pair=pair_s, fmt="%f")
...
if __name__ == "__main__":
    m, db, inb, out, use_cpu = parse_args()
    FFDB=db
    FFindexDB = namedtuple("FFindexDB", "index, data")
    ffdb = FFindexDB(read_index(FFDB+'_pdb.ffindex'),
                     read_data(FFDB+'_pdb.ffdata'))

    pred = Predictor(model_dir=m, use_cpu=use_cpu)

    with open(f"{inb}/IDX.uniq") as f:
        ins = f.read().splitlines()

    for i in ins:
        print(f"Predicting {i}...", end="")
        pf = f"{inb}/{i}"
        res = pred.predict(f"{pf}_.msa0.a3m", f"{out}/{i}", f"{pf}_.hhr", f"{pf}_.atab")
        print("Done!")
